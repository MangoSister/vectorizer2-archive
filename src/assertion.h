...
using AssertLevelFatal = debug_assert::level<1>;
using AssertLevelRegular = debug_assert::level<2>;

#ifdef NDEBUG
#define HANDLE_ASSERT_LEVEL 1
#else
#define HANDLE_ASSERT_LEVEL 2
#endif

struct AssertHandler :
    debug_assert::default_handler,                  // it uses the default handler
    debug_assert::set_level<HANDLE_ASSERT_LEVEL>    // and this level
{
};

#define ASSERT(expr)  DEBUG_ASSERT(expr, AssertHandler{}, AssertLevelRegular{})
#define ASSERT_FATAL(expr)  DEBUG_ASSERT(expr, AssertHandler{}, AssertLevelFatal{})
#define ASSERT_UNREACHABLE(...)  do { DEBUG_UNREACHABLE(AssertHandler{}, AssertLevelFatal{}); } while (false);
