...
public:
	...
	// Vectorizer.
    Frustum buildFrustum(const Point2i &pixel) const {
        Point2 offsets[5] = {
            // center
            Point2((Float)pixel.x + 0.5f,   (Float)pixel.y + 0.5f),
            // corners
            Point2((Float)pixel.x,		    (Float)pixel.y + 1.0f),
            Point2((Float)pixel.x + 1.0f,	(Float)pixel.y + 1.0f),
            Point2((Float)pixel.x + 1.0f,	(Float)pixel.y),
            Point2((Float)pixel.x,	    	(Float)pixel.y)
        };
        Vector dirs[5];
        const Transform &worldTrans = m_worldTransform->eval(0.0);
        for (uint32_t i = 0; i < 5; ++i) {
            Point corner(
                offsets[i].x * m_invResolution.x,
                offsets[i].y * m_invResolution.y,
                0.0);
            corner = m_sampleToCamera(corner);
            // cornerDirs[i] = normalize(Vector(corner)); // No need to normalize.
            dirs[i] = Vector(corner);
            dirs[i] = worldTrans(dirs[i]);
        }
        Point apex = worldTrans.transformAffine(Point(0.0));
        return Frustum(apex, dirs[0], &dirs[1], getNearClip(), FrustumAABBOption::eExact);
    }
