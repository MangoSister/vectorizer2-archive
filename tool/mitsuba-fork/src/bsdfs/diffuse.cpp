...
public:
    ...
    Spectrum evalPolyIntegral(const BSDFPolyIntegralContext &context) const {
        Spectrum rd = m_reflectance->eval(context.its);
        ...
        for (uint32_t i = 0; i < leafCount; ++i) {
            ...
            irradiance += sphericalPolyFormFactor(localPoly.data(), region.vertCount);
        }
        return rd * (Float)irradiance * INV_PI;
    }

    void evalPolyIntegralGrad(const BSDFPolyIntegralGradContext &context, vtrz::DVector3 &bsdfIntGrad) const {
        vtrz::DVector3 rd;
        m_reflectance->eval(context.its).toLinearRGB(rd.x.value, rd.y.value, rd.z.value);
        std::vector<vtrz::DVector3> localPoly;

        uint32_t leafCount = context.vect.leafCount();
        vtrz::dfloat irradiance(0.0f);
        for (uint32_t i = 0; i < leafCount; ++i) {
            ...
            vtrz::dfloat irr;
            sphericalPolyFormFactor(localPoly.data(), region.vertCount, irr);
            irradiance += irr;
        }
        bsdfIntGrad = rd * irradiance * INV_PI;
    }

    void evalGrad(const BSDFEvalGradContext &ctx, vtrz::DVector3 &F) const {
        if (ctx.wi.z.value <= 0.0 || ctx.wo.z.value <= 0.0) {
            F = vtrz::DVector3(0.0);
            return;
        }
        vtrz::DVector3 rd;
        m_reflectance->eval(ctx.its).toLinearRGB(rd.x.value, rd.y.value, rd.z.value);
        F = rd * INV_PI * ctx.wo.z;
    }
    ...
    void applyGrad(uint32_t globalChannelOffset) {
        m_reflectanceGrad = vtrz::DVector3(0.0f);
        ...
        for (const auto &token : tokens) {
            ...
            SAssert(token[0] == 'r');
            if (token[0] == 'r') {
                SAssert(token[1] == 'r' || token[1] == 'g' || token[1] == 'g' || token[1] == 'a');
                SAssert(token[2] == '+' || token[2] == '-');
                if (token[1] == 'r') {
                    if (token[2] == '+')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(0.0, globalChannelOffset++), vtrz::dfloat(), vtrz::dfloat());
                    else if (token[2] == '-')
                        m_reflectanceGrad = vtrz::DVector3(-vtrz::dfloat(0.0, globalChannelOffset++), vtrz::dfloat(), vtrz::dfloat());
                } else if (token[1] == 'g') {
                    if (token[2] == '+')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(), vtrz::dfloat(0.0, globalChannelOffset++), vtrz::dfloat());
                    else if (token[2] == '-')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(), -vtrz::dfloat(0.0, globalChannelOffset++), vtrz::dfloat());
                } else if (token[1] == 'b') {
                    if (token[2] == '+')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(), vtrz::dfloat(), vtrz::dfloat(0.0, globalChannelOffset++));
                    else if (token[2] == '-')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(), vtrz::dfloat(), -vtrz::dfloat(0.0, globalChannelOffset++));
                } else if (token[1] == 'a') {
                    if (token[2] == '+')
                        m_reflectanceGrad = vtrz::DVector3(vtrz::dfloat(0.0, globalChannelOffset++));
                    else if (token[2] == '-')
                        m_reflectanceGrad = vtrz::DVector3(-vtrz::dfloat(0.0, globalChannelOffset++));
                }
            }
...
private:
    ...
    std::string m_grad;
    // ignore values.
    vtrz::DVector3 m_reflectanceGrad;
};
