...
Spectrum BSDF::evalPolyIntegral(const BSDFPolyIntegralContext &context) const {
	NotImplementedError("evalPolyIntegral");
}

void BSDF::evalPolyIntegralGrad(const BSDFPolyIntegralGradContext &context, vtrz::DVector3 &bsdfIntGrad) const {
    NotImplementedError("evalPolyIntegralGrad");
}

void BSDF::evalGrad(const BSDFEvalGradContext &ctx, vtrz::DVector3 &F) const {
    NotImplementedError("evalGrad");
}

uint32_t BSDF::gradDim() const { return 0; }

void BSDF::applyGrad(uint32_t globalChannelOffset) { }
