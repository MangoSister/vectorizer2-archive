...
public:
    ...
    //! @{ \name Vectorizer
    // =============================================================
    virtual Spectrum evalPolyIntegral(const BSDFPolyIntegralContext &context) const;

    // TODO: walk-around weird compiling error.
    virtual void evalPolyIntegralGrad(const BSDFPolyIntegralGradContext &context, vtrz::DVector3 &bsdfIntGrad) const;

    virtual void evalGrad(const BSDFEvalGradContext &ctx, vtrz::DVector3 &F) const;

    virtual uint32_t gradDim() const;

    virtual void applyGrad(uint32_t globalChannelOffset);

    uint16_t getLabel() const { return m_label; }

    void setLabel(uint16_t label) { m_label = label; }
